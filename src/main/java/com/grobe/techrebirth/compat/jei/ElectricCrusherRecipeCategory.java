...
public class ElectricCrusherRecipeCategory implements IRecipeCategory<CrushingRecipe> {
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, CrushingRecipe recipe, IFocusGroup focuses) {
        ...
        //chance output
        builder.addSlot(RecipeIngredientRole.OUTPUT, 84, 37).setBackground(slotDrawable, -1, -1).addItemStack(recipe.getChanceOutput());
    }

    @Override
    public void draw(CrushingRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics gg, double mouseX, double mouseY) {
        ...
        float chancePerc = recipe.getChanceRate() * 100f;

        //Chance Percentage
        Font font = Minecraft.getInstance().font;
        gg.drawString(font, Component.literal(chancePerc + "% chance"),70,55, 0xFFFFFFFF, true);
    }
}
